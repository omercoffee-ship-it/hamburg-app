...
function ShiftApp({ reports, onSubmit, onBack, staffMode, ownerMode, editMode }) {
  ...
  // Auto-save key based on shift and date
  const getSaveKey = (sh, date) => `hamburg_draft_${sh}_${date}`;
  ...
  const [form, setForm] = useState(() => {
    if (editMode?.existing) {
      ...
      return {
        ...
        privateDeliveries: String(e.deliveries?.private||""),
        taDeliveries: String(e.deliveries?.ta||""),
        ...
        topDishes: e.topDishes?.length ? [...e.topDishes,...Array(Math.max(0,2-e.topDishes.length)).fill("")] : ["",""],
  ...
  // Load draft from localStorage when shift/date selected
  const loadDraft = (sh, dateKey) => {
    try {
      const saved = localStorage.getItem(getSaveKey(sh, dateKey));
  ...
  const up = (f,v) => {
    setForm(p => {
      const newForm = {...p, [f]:v};
      if (shift && activeDate) {
        try { localStorage.setItem(getSaveKey(shift, dateKey(activeDate)), JSON.stringify(newForm)); } catch(e) {}
      }
      return newForm;
    });
  };
  ...
  const today     = new Date();
  const activeDate= (() => { 
    if (!selDate || selDate==="today") return today;
    // selDate is now a date key like "2026-06-15"
  ...
  // up defined above with auto-save
  const upC = (arr,i,f,v)=> setForm(p=>{
    const c=[...p[arr]];c[i]={...c[i],[f]:v};
    const newForm={...p,[arr]:c};
    if(shift&&activeDate){try{localStorage.setItem(getSaveKey(shift,activeDateKey),JSON.stringify(newForm));}catch(e){}}
    return newForm;
  });
  const upD = (i,v)  => setForm(p=>{
    const d=[...p.topDishes];d[i]=v;
    const newForm={...p,topDishes:d};
    if(shift&&activeDate){try{localStorage.setItem(getSaveKey(shift,activeDateKey),JSON.stringify(newForm));}catch(e){}}
    return newForm;
  });
  ...
  const handleSubmit = () => {
    ...
    try { localStorage.removeItem(getSaveKey(shift, editMode?.dateKey || activeDateKey)); } catch(e) {}
    setSubmitted(true);
  };
  ...
  if (!shift) return (
    <div style={S.app}>
      ...
      <div style={{padding:20,display:"flex",flexDirection:"column",gap:14,alignItems:"center"}}>
        ...
        {["lunch","dinner"].map(sh=>(
          <button key={sh} style={{...S.shiftBtn,background:sh==="lunch"?"#e8f4fd":"#1a2e1a",borderColor:sh==="lunch"?"#2c5f8a":"#4a7c3f"}}
            onClick={()=>{
              const dk=selDate&&selDate!=="today"?selDate:activeDateKey;
              const draft=loadDraft(sh,dk);
              if(draft) setForm(draft);
              setShift(sh);
            }}>
            <span style={{fontSize:32}}>{sh==="lunch"?"🌅":"🌙"}</span>
            <span style={{fontSize:18,fontWeight:700,color:sh==="lunch"?"#2c5f8a":"#9cb89a"}}>{sh==="lunch"?"משמרת צהריים":"משמרת ערב"}</span>
            {loadDraft(sh,selDate&&selDate!=="today"?selDate:activeDateKey)&&(
              <span style={{fontSize:11,background:"#ff9800",color:"#fff",borderRadius:10,padding:"2px 8px"}}>טיוטה שמורה ↩</span>
            )}
          </button>
        ))}
      </div>
  ...
  return (
    <div style={S.app}>
      ...
      <div style={{background:shift==="lunch"?"#e8f4fd":"#1a2e1a",padding:"8px 12px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{display:"flex",gap:4}}>
          {[{n:1,label:"פרטים"},{n:2,label:"משלוחים"},{n:3,label:"ביטולים"},{n:4,label:"מנות"}].map(({n,label})=>(
            <button key={n} onClick={()=>setStep(n)} style={{
              padding:"4px 8px",borderRadius:8,border:"none",cursor:"pointer",fontSize:11,fontWeight:step===n?700:400,
              background:step===n?shiftColor:"rgba(255,255,255,0.15)",
              color:step===n?(shift==="lunch"?"#fff":"#fff"):"rgba(255,255,255,0.6)"
            }}>{n}. {label}</button>
          ))}
        </div>
        <span style={{color:shift==="lunch"?"#2c5f8a":"#9cb89a",fontWeight:700,fontSize:13}}>{shift==="lunch"?"🌅":"🌙"}</span>
      </div>
