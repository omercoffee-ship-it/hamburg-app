...
const dbGetReports = async () => {
  ...
  data.forEach(row => {
    ...
    result[row.date_key][row.shift] = {
      ...
      deliveries: { total: row.deliveries_total, wolt: row.deliveries_wolt },
...
// ─── EMPTY FORM ───────────────────────────────────────────────
const emptyForm = () => ({
  manager:"", diners:"", avgPerDiner:"", totalDeliveries:"", woltDeliveries:"",
  staffMeals:"",
  ...
  operationalIssues:[], operationalNote:"",
});
...
function OwnerDashboard({ reports, manualRev, setManualRev, onSubmit, onSaveRevenue, onBack, noPinRequired }) {
  ...
  const todayRev   = manualRev[todayK]||null;
  ...
  if (view==="shift") return <ShiftApp reports={reports} onSubmit={onSubmit} onBack={()=>setView("dashboard")} ownerMode />;
    ...
    <div style={S.app}>
      ...
      <div style={S.card}>
        <div style={{...S.cardHdr, background:"#1a2e1a"}}>
          ...
          <span style={{color:"#fff",fontWeight:700,fontSize:15}}>היום — {DAYS_HE[today.getDay()]}, {today.getDate()}/{today.getMonth()+1}</span>
        </div>
        <div style={S.cardBody}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              {editRev===todayK ? <>
                <button style={S.saveBtn} onClick={()=>saveRev(todayK)}>✓</button>
                <input autoFocus style={{...S.revInput,width:100}} value={tempRev} onChange={e=>setTempRev(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")saveRev(todayK);}} placeholder="פדיון" />
              </> : <>
                <button style={S.editBtn} onClick={()=>{setEditRev(todayK);setTempRev(todayRev?String(todayRev):"");}}>✏️</button>
          ...
          <span style={S.sub}>וולט: {todayWolt} | פרטי: {todayDel-todayWolt} | סה״כ: {todayDel} 🛵</span>
        </div>
      </div>
      ...
      <GreenCard title={`מכירות השבוע, ${today.getDate()}/${today.getMonth()+1}`}>
        ...
        <span style={S.sub}>וולט: {weekWolt} | פרטי: {weekDel-weekWolt} | סה״כ: {weekDel} 🛵</span>
      </GreenCard>
      ...
      <GreenCard title={`${mn} עד כה`}>
        ...
        <span style={S.sub}>וולט: {monthWolt} | פרטי: {monthDel-monthWolt} | סה״כ: {monthDel} 🛵</span>
      </GreenCard>
...
function ShiftApp({ reports, onSubmit, onBack, staffMode, ownerMode, editMode }) {
  ...
  const handleSubmit = () => {
    const data = {
      ...
      topDishes: form.topDishes.map((d,i)=>d==="אחר"?(form.topDishesOther?.[i]||"אחר"):d).filter(Boolean),
      operationalIssues: [...form.operationalIssues, ...(form.operationalNote?[form.operationalNote]:[])],
    };
  ...
  const canNext2 = form.totalDeliveries !== "";
    ...
    <div style={S.app}>
      ...
      <div style={{padding:"12px 12px 40px"}}>
        ...
        {step===2&&<div style={S.section}>
          ...
          <Label>סה״כ משלוחים במשמרת</Label>
          <input style={S.inp} type="number" inputMode="numeric" placeholder="סה״כ כל המשלוחים" value={form.totalDeliveries} onChange={e=>up("totalDeliveries",e.target.value)}/>
          <Label>מתוכם — כמה וולט?</Label>
          <input style={S.inp} type="number" inputMode="numeric" placeholder="משלוחי וולט בלבד" value={form.woltDeliveries} onChange={e=>up("woltDeliveries",e.target.value)}/>
          {form.totalDeliveries&&form.woltDeliveries&&(
            <div style={{background:"#f0f7f0",border:"1px solid #c3e0c3",borderRadius:8,padding:"10px 14px",display:"flex",justifyContent:"space-around",fontSize:14,marginTop:10}}>
              <span>וולט: <b>{form.woltDeliveries}</b></span>
              <span>פרטי: <b>{Math.max(0,(parseInt(form.totalDeliveries)||0)-(parseInt(form.woltDeliveries)||0))}</b></span>
              <span>סה״כ: <b>{form.totalDeliveries}</b></span>
            </div>
          )}
          <div style={{display:"flex",gap:8,marginTop:16}}>
        ...
        {step===4&&<div style={S.section}>
          ...
          {OPS_OPTIONS.map(opt=>(
            <div key={opt} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:"1px solid #f5f5f5",cursor:"pointer"}} onClick={()=>togOps(opt)}>
          ...
          {form.operationalIssues.includes("אחר")&&<input style={{...S.inp,marginTop:8}} placeholder="פרט..." value={form.operationalNote} onChange={e=>up("operationalNote",e.target.value)}/>}
...
function ShiftCard({ title, shift }) {
  ...
  return (
    <div style={{margin:"12px 12px 0",background:"#fff",borderRadius:10,overflow:"hidden",boxShadow:"0 1px 4px rgba(0,0,0,0.08)",paddingBottom:12}}>
      ...
      <R label="פרטי" val={totalDel-woltDel}/>
      ...
      {shift.operationalIssues?.length>0&&<><div style={{height:1,background:"#f0f0f0",margin:"4px 14px"}}/><div style={{padding:"6px 14px 2px",color:"#888",fontSize:13}}>בעיות תפעול:</div>{shift.operationalIssues.map((o,i)=><div key={i} style={{padding:"3px 14px",fontSize:13,color:"#e67e22"}}>⚠️ {o}</div>)}</>}
    </div>
  );
}
        ...
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,direction:"rtl"}}>
          <thead>
            <tr style={{borderBottom:"2px solid #eee"}}>
              ...
              <th style={{textAlign:"center",padding:"4px 3px",color:"#2c5f8a",fontWeight:600,fontSize:10}}>וולט צ׳</th>
              <th style={{textAlign:"center",padding:"4px 3px",color:"#666",fontWeight:600,fontSize:10}}>פרטי צ׳</th>
              <th style={{textAlign:"center",padding:"4px 3px",color:"#2c5f8a",fontWeight:600,fontSize:10}}>וולט ע׳</th>
              <th style={{textAlign:"center",padding:"4px 3px",color:"#666",fontWeight:600,fontSize:10}}>פרטי ע׳</th>
              <th style={{textAlign:"center",padding:"4px 3px",color:"#111",fontWeight:700,fontSize:10}}>סה״כ</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r,i)=>{
              ...
              return (
                <tr key={i} style={{borderBottom:"1px solid #f5f5f5",opacity:isEmpty?0.3:1}}>
                  ...
                  <td style={{padding:"7px 3px",textAlign:"center",color:"#2c5f8a"}}>{isEmpty?"—":r.lW}</td>
                  <td style={{padding:"7px 3px",textAlign:"center",color:"#888"}}>{isEmpty?"—":r.lP}</td>
                  <td style={{padding:"7px 3px",textAlign:"center",color:"#2c5f8a"}}>{isEmpty?"—":r.dW}</td>
                  <td style={{padding:"7px 3px",textAlign:"center",color:"#888"}}>{isEmpty?"—":r.dP}</td>
                  <td style={{padding:"7px 3px",textAlign:"center",fontWeight:700,color:tc,fontSize:13}}>{isEmpty?"—":r.total}</td>
                </tr>
              );
            })}
            <tr style={{borderTop:"2px solid #eee",fontWeight:700,background:"#f9f9f9"}}>
              <td style={{padding:"7px 6px",color:"#111"}}>סה״כ</td>
              <td style={{padding:"7px 3px",textAlign:"center",color:"#2c5f8a"}}>{tot.lW}</td>
              <td style={{padding:"7px 3px",textAlign:"center",color:"#888"}}>{tot.lP}</td>
              <td style={{padding:"7px 3px",textAlign:"center",color:"#2c5f8a"}}>{tot.dW}</td>
              <td style={{padding:"7px 3px",textAlign:"center",color:"#888"}}>{tot.dP}</td>
              <td style={{padding:"7px 3px",textAlign:"center",color:"#111",fontSize:14}}>{tot.total}</td>
            </tr>
          </tbody>
        </table>
    ...
    <div style={S.app}>
      ...
      <div style={{padding:16}}>
        ...
        <div style={{marginTop:16,background:"#fff",borderRadius:12,padding:16,boxShadow:"0 1px 4px rgba(0,0,0,0.08)",textAlign:"center"}}>
          <div style={{fontSize:13,color:"#888"}}>📎 את תבנית האקסל הורד מהשיחה עם Claude</div>
        </div>
